...
<div class="container">
	...
	<div class="row my-5 gyoon-link" align="center">
	  ...
	  <form method="get" action="/product/productSale.do" name="productSearchForm" class="mx-2"> 
	  ...
	  <div class="d-flex justify-content-center my-5">
	    <div class=" fs-4">
		  ...
		  <c:if test="${pageVO.prev}">
		    <a href="productSale.do?type=${pageVO.cri.type}&keyword=${pageVO.cri.keyword}&pageNum=1&amount=${pageVO.cri.amount}"><i class="bi bi-chevron-double-left"></i></a> 
		    <a href="productSale.do?type=${pageVO.cri.type}&keyword=${pageVO.cri.keyword}&pageNum=${pageVO.startPage - 1}&amount=${pageVO.cri.amount}"><i class="bi bi-caret-left-square"></i></a> 
		  </c:if>

		  <c:forEach var="num" begin="${pageVO.startPage}" end="${pageVO.endPage}">
			<a href="productSale.do?type=${pageVO.cri.type}&keyword=${pageVO.cri.keyword}&pageNum=${num}&amount=${pageVO.cri.amount}" class="${pageVO.cri.pageNum == num ? 'border border-dark rounded-3 px-2' : ''}">${num}</a> 			
		  </c:forEach>
		  
		  <c:if test="${pVO.next}">
		    <a href="productSale.do?type=${pageVO.cri.type}&keyword=${pageVO.cri.keyword}&pageNum=${pageVO.startPage + 1}&amount=${pageVO.cri.amount}"><i class="bi bi-caret-right-square"></i></a> 
		    <a href="productSale.do?type=${pageVO.cri.type}&keyword=${pageVO.cri.keyword}&pageNum=${pageVO.realEnd}&amount=${pageVO.cri.amount}"><i class="bi bi-chevron-double-right"></i></a> 
		  </c:if>
