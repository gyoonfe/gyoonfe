	  ...
	  <div class="gyoon-pd-mg">
		  ...
		  <div class="row my-5">		  
			<div class="col-md-12">
			    ...
			    <p class="fs-1 mt-5 gyoon-redressed">Board Detail</p>
			    ...
			    <div class="scrollspy-example gyoon-content mt_100 mb_100">${bVO.content}</div>
			  ...
			  <form method="post" action="/admin/boardRepWrite.do" name="repWrite">
				...
				<c:forEach var="repVO" items="${reply_list}">
			      <tr>
			        ...
			        <th>
			          ...
			          <br>
			          <a class="text-white" href="boardRepDelete.do?repno=${repVO.repno}&bno=${bVO.bno}&pageNum=${cri.pageNum}&amount=${cri.amount}" onClick="return confirm('You sure to delete this Reply?')">Delete</a>
			        </th>	        
			      </tr>
			    </c:forEach>
					...
					<c:forEach var="repVO" items="${reply_list}" begin="0" end="9">
				      <tr>
				        ...
				        <th>
				          ...
				          <br>
				          <a href="boardRepDelete.do?repno=${repVO.repno}&bno=${bVO.bno}&pageNum=${cri.pageNum}&amount=${cri.amount}" onClick="return confirm('You sure to delete this Reply?')">Delete</a>
				        </th>	        
				      </tr>
				    </c:forEach>
				</c:when>
...
function checkBoardUpdate(){
	if(confirm('Are you sure update this post?')){
		location.href="/admin/boardUpdate.do?bno=<c:out value="${bVO.bno}"/>&pageNum=${cri.pageNum}&amount=${cri.amount}&userid=${bVO.userid}";
	}else{
		return false;
	}
}
...
function checkBoardDelete(){
	if(confirm('Are you sure delete this post?')){
		location.href="boardDelete.do?bno=<c:out value="${bVO.bno}"/>&pageNum=${cri.pageNum}&amount=${cri.amount}";
	}else{
		return false;
	}
}
